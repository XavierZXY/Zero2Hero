...
class TextGenerator:
    def __init__(
        ...
        self.tokenizer = Tokenizer(
            tokenizer_model=self.tokenizer_model_path
        )  # 根据指定的路径加载分词器
    ...
    def sample(
        ...
        start_ids = self.tokenizer.encode(
            start, bos=True, eos=False
        )  # bos=True 表示加上句首标记，eos=False 表示不加句尾标记
...
if __name__ == "__main__":
    ...
    samples = generator.sample(
        start=args.prompt, num_samples=3, max_new_tokens=256
    )  # 生成 3 个样本
