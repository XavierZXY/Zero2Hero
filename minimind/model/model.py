...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.top_k > 1 and self.norm_topk_prob:
            ...
            topk_weight = topk_weight / denominator  # 归一化topk概率

        if self.training and self.alpha > 0.0:
            # 计算一个辅助损失，用于在训练过程中平衡不同专家的使用。这个辅助损失基于专家评分和 topk 选择的索引，
            # 通过不同的计算方式来调整专家的使用频率。
        ...
        **args,
        ...
        for l_id, layer in enumerate(self.layers):
            h, past_kv = layer(
                h,
                pos_cis,
                past_key_value=past_key_values[l_id],
                use_cache=use_cache,
            )
            past_kvs.append(past_kv)
        logits = self.output(self.norm(h))
        aux_loss = sum(
            lay.feed_forward.aux_loss
            for lay in self.layers
            if isinstance(lay.feed_forward, MOEFeedForward)
        )
        ...
        **args,
        ...
        while input_ids.shape[1] < max_new_tokens - 1:
            if first_seq or not use_cache:
                out, first_seq = (
                    self(input_ids, past_key_values=past_kvs, use_cache=use_cache),
                    False,
                )
            else:
                out = self(
