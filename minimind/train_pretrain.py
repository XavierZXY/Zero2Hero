...
def Logger(content):
    """日志打印函数，如果是DDP模式则只有rank=0的进程打印日志
...
def get_lr(current_step, total_steps, lr):
    ...
    Args:
        current_step (_type_): _description_
...
def init_model(lm_config):
    ...
    Logger(
        f"LLM总参数量：{sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.3f} M"
    )
...
if __name__ == "__main__":
    ...
    args.wandb_run_name = f"MiniMind-Pretrain-Epoch-{args.epochs}-BatchSize-{args.batch_size}-UseMoe-{args.use_moe}-LearningRate-{args.learning_rate}"
